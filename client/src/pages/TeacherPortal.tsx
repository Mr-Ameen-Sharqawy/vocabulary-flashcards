...
type StudentGrade = "grade4" | "grade5" | "grade6";
...
export default function TeacherPortal({ onBack }: TeacherPortalProps) {
  ...
  const [allowedGrades, setAllowedGrades] = useState<StudentGrade[]>(["grade4", "grade5"]);
  ...
  function toggleGrade(grade: StudentGrade) {
    setAllowedGrades(current => current.includes(grade) ? (current.length > 1 ? current.filter(value => value !== grade) : current) : [...current, grade]);
  }

  function updateStudentGrades(studentId: number, current: StudentGrade[], grade: StudentGrade) {
